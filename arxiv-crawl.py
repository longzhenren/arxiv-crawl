...
def parse_metadata(xml_metadata, output_file):
    ...
    if len(xml_tree.getElementsByTagName('comments')) == 0:
        ...
        comments = xml_tree.getElementsByTagName(
            'comments')[0].childNodes[0].nodeValue
    if len(xml_tree.getElementsByTagName('doi')) == 0:
        doi = None
    else:
        doi = xml_tree.getElementsByTagName('doi')[0].childNodes[0].nodeValue
    abstract = xml_tree.getElementsByTagName(
        'abstract')[0].childNodes[0].nodeValue
    raw_versions = xml_tree.getElementsByTagName('version')
    versions = []
    for index, i in enumerate(raw_versions):
        versions.append({'version': "v{0}".format(index),
                        'created': i.childNodes[0].childNodes[0].nodeValue})
    update_date = xml_tree.getElementsByTagName(
        'datestamp')[0].childNodes[0].nodeValue
    authors = xml_tree.getElementsByTagName(
        'authors')[0].childNodes[0].nodeValue
    year = update_date[0:4]
    ass = authors
...
def download_metadata(arxiv_id, output_file):
    try:
        ...
        print(
            "ERROR:error in downloading metadata of arxiv_id {0}".format(arxiv_id))
...
def parse_argument(args):
    ...
    parser.add_argument("-s", "--start-yymm", type=str, default="2101",
                        help="The start (contain) year and month, in format yymm, like 1101 (rep Jan 2011)")
    parser.add_argument("-e", "--end-yymm", type=str, default="2112",
...
if __name__ == "__main__":
    ...
    try:
        raw_submit_number = requests.get(
            "https://arxiv.org/stats/get_monthly_submissions").content.decode()
        submit_number = [int(x.split(',')[1])
                         for x in raw_submit_number.split()[1:]]
    except:
        logging.error(
            "Fail to connect to arxiv. Please check your network status")
        exit(-1)
        ...
        for i in range(start_id, end_id + 1):
            for j in range(1, submit_number[i] + 1):
                ...
                if cnt > args_dict.maximum:
                    break
                if i < 282:
                    download_arxiv_id_list.append(
                        "{:02}{:02}.{:04}".format((i - 102) // 12, id2month(i), j))
                else:
                    download_arxiv_id_list.append(
                        "{:02}{:02}.{:05}".format((i - 102) // 12, id2month(i), j))
    ...
    output_file = "arxiv_{0}.json".format(
